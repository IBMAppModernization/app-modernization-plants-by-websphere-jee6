...
if [ ! -f k8s/pbw-mysql-credentials.yaml ]; then
  echo "Fatal error: Can't find k8s secrets file on path k8s/pbw-mysql-credentials.yaml"
  exit 1
fi

# store the whole response with the status at the and
HTTP_RESPONSE=$(curl --silent --write-out "HTTPSTATUS:%{http_code}" -X POST -H "Content-Type: text/plain" -d @k8s/pbw-mysql-credentials.yaml $1)
